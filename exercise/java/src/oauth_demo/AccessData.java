...
public class AccessData {
    ...
    public AccessData(String accessToken, String userId, String saveMessage) {
        ...
        ConsoleLogger.debug("Access token file is %s", this.ACCESS_TOKEN_FILE);
    }
    public AccessData() {
        this.accessToken = "";
        this.userId = "";
    ...
    public void load() throws IOException {
        ConsoleLogger.debug("Attempting to load access data from token file %s", this.ACCESS_TOKEN_FILE);
        byte[] encoded = Files.readAllBytes(Paths.get(this.ACCESS_TOKEN_FILE));
        String jsonFileContents = new String(encoded, StandardCharsets.UTF_8);
        ConsoleLogger.debug("Access data successfully loaded from token file %s", this.ACCESS_TOKEN_FILE);
    ...
    public void save() throws IOException {
        ConsoleLogger.debug("Attempting to save access data to token file %s", this.ACCESS_TOKEN_FILE);
        ...
        ConsoleLogger.debug("About to save access data to token file %s", this.ACCESS_TOKEN_FILE);
        PrintWriter printWriter = new PrintWriter(this.ACCESS_TOKEN_FILE);
        printWriter.println(jsonText);
        printWriter.close();
        ConsoleLogger.debug("Access data successfully saved to token file %s", this.ACCESS_TOKEN_FILE);
    }
